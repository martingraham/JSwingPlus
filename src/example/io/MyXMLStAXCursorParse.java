...
import example.io.MySimpleSchemaTagConstants.MyTags;
...
public class MyXMLStAXCursorParse extends AbstractStAXCursorParse {

	static final Logger LOGGER = Logger.getLogger (MyXMLStAXCursorParse.class);
	
    private final EnumSet<MyTags> startTags = MySimpleSchemaTagConstants.startTags;
    private final EnumSet<MyTags> endTags = MySimpleSchemaTagConstants.endTags;
    ...
    public MyXMLStAXCursorParse (final Object populateMe) {
        super (populateMe);
    ...
    public static void main (final String[] args) {
        ...
        final MyXMLStAXCursorParse staxParse = new MyXMLStAXCursorParse (treeList);
        LOGGER.info ("Filename: "+args[0]);
        try {
            staxParse.parse (DataPrep.getInstance().getPossiblyZippedInputStream (args[0], true));
        } catch (final Exception exc) {
        	LOGGER.error ("error in cursorparse parse: " + exc.toString());
        }
	...
	protected void specificProcessStartElement (final String tagName, final Map<String, String> attrStore){
        	...
        	final MyTags tag = MySimpleSchemaTagConstants.tagMatcher.get (tagName);
        	
            if (startTags.contains (tag)) {
            	
            	final List<TreeModel> treeList = (List<TreeModel>) populateMe;
               ...
               switch (tag) {
                ...
                case TREE_NODE:
                    ...
                    final TreeModel tModel = treeList.get (currentTreeIndex);
                    MutableTreeNode newNode;
                    
                    if (!parentStack.empty()) {
                       final MutableTreeNode parentNode = parentStack.peek();
                       newNode = new DefaultMutableTreeNode ();
                       final Object userObject = userObjects.get (cid);
	...
	protected void specificProcessEndElement (final String tagName, final Map<String, String> attrStore) {
            ...
            if (endTags.contains (tag)) {
            	...
            	final Collection<TreeModel> treeList = (Collection<TreeModel>) populateMe;
               ...
               case NAME:
                  ...
                  final TaxonUserObject tuo = new TaxonUserObject (text, rankIDs.get (attrStore.get ("RANK_IDREF")));
                  userObjects.put (nID, tuo);
                  break;

               case TREE_NAME:
            	  final MutableTreeNode root = new DefaultMutableTreeNode2 (rootUserObj);
            	  parentStack.add (root);
            	  final TreeModel tModel = new DefaultTreeModel (root);
            	  treeList.add (tModel);
