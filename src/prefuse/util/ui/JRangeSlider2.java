 ...
 * @author Colin Combe
 */
public class JRangeSlider2 extends JComponent 
...
{   
	 ...
	 * 
	 */
	private static final long serialVersionUID = -3109538456335513738L;
    ...
    protected static int ARROW_SZ = 16;
    ...
    protected final static Color translucentWhite = new Color (255, 255, 255, 192);
    protected final static Color translucentBlack = new Color (0, 0, 0, 192);
    protected Border thumbBorder;
     ...
     */
    public JRangeSlider2(final int minimum, final int maximum, final int lowValue, final int highValue, final int orientation) {
     ...
     */
    public JRangeSlider2(final int minimum, final int maximum, final int lowValue, final int highValue, final int orientation, final int direction) {
        this(new DefaultBoundedRangeModel(lowValue, highValue - lowValue, minimum, maximum), 
     ...
     * @param direction - Is the slider left-to-right/top-to-bottom or right-to-left/bottom-to-top
     */
    public JRangeSlider2(final BoundedRangeModel model, final int orientation, final int direction) {
        super.setFocusable(true);
        this.model = model;
        this.orientation = orientation;     
        this.direction = direction;
        this.setThumbBorder (BorderFactory.createBevelBorder (BevelBorder.RAISED));
    ...
    public void setLowValue(final int lowValue) {
        final int e = (model.getValue()-lowValue)+model.getExtent();
    ...
    public final Border getThumbBorder () {
		return thumbBorder;
	}

	public final void setThumbBorder (Border thumbBorder) {
	...
	public void paintComponent(final Graphics g) {        
        ...
        //System.out.println ("bounds: "+bounds+", insets: "+getInsets());
        GraphicsUtil.adjustRectangle (bounds, getInsets());
        //System.out.println ("bounds: "+bounds);
        final int width = (int)bounds.getWidth() - 1;
        final int height = (int)bounds.getHeight() - 1;
        ...
        if (orientation == VERTICAL) {  
            if (direction == LEFTRIGHT_TOPBOTTOM) {
                ...
                if (thumbColor != null && thumbColor.getAlpha() > 0) {
                ...
                if (thumbBorder != null) {
                	thumbBorder.paintBorder (this, g2, 0, min, width + 1, max-min);
                }
                ...
                if (thumbBorder != null) {
                	thumbBorder.paintBorder (this, g2, 0, max, width + 1, min-max);
                }
     ...
     */
    protected void paintArrow(final Graphics2D g2, final double x, final double y, int w, int h,
                              final boolean topDown)
    {
        final int intX = (int)(x+0.5);
        final int intY = (int)(y+0.5);
     ...
     */
    public int toScreen(final int xOrY) {
    	final Dimension sz = GraphicsUtil.adjustDimension (getSize(), getInsets());
        final int min = getMinimum();
        double scale;
        if (orientation == VERTICAL) {
            scale = (sz.height - (2 * ARROW_SZ)) / (double) (getMaximum() - min);           
     ...
     * Converts from a range value to screen coordinates.
     */
    public double toScreenDouble(final int xOrY) {
        final Dimension sz = GraphicsUtil.adjustDimension (getSize(), getInsets());
        final int min = getMinimum();
        double scale;
        if (orientation == VERTICAL) {
    ...
    public static final int PICK_LEFT_OR_TOP = 1;
    public static final int PICK_THUMB = 2;
    public static final int PICK_RIGHT_OR_BOTTOM = 3;
    int pick;
    int pickOffsetLow;
    int pickOffsetHigh;
    int mouse;

    public int pickHandle (final int xOrY) {
        final int min = toScreen(getLowValue());
        final int max = toScreen(getHighValue());
        int pick = PICK_NONE;
    ...
    public void mouseDragged(final MouseEvent e) {
        requestFocus();
        final int value = (orientation == VERTICAL) ? e.getY() : e.getX();
        
        final int minimum = getMinimum();
        final int maximum = getMaximum();
        final int lowValue = getLowValue();
        final int highValue = getHighValue();
    ...
    public void keyPressed (final KeyEvent e) {
        ...
        final boolean l = (kc == KeyEvent.VK_LEFT);
        final boolean r = (kc == KeyEvent.VK_RIGHT);
        
        if (d || u || l || r) {
	        // my change
	        if (u ^ d && v && direction == JRangeSlider2.LEFTRIGHT_TOPBOTTOM) {
	        	u = !u;
	        	d = !d;
	        }
