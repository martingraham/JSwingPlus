 ...
 */
public class ParCoord_Old_UI extends TableUI {
    private static final StringBuilder BASELINE_COMPONENT_KEY =
        new StringBuilder ("Table.baselineComponent");
    
    final private static Class<ParCoord_Old_UI> CLASS_OBJ = ParCoord_Old_UI.class;
    ...
    protected transient Dimension lineDrawCounter = new Dimension (0, 0);
    protected transient long paintTimeOut = (long) (50 * 1E6); // milliseconds * 1E6 = nanoseconds    
		...
		public void mouseReleased (final MouseEvent mEvent) {
			...
			if ((initialModifiers & Event.ALT_MASK) > 0) {
				...
				if (dragCol == pressedCol) {
	             	...
	             	rsm.clearSelection ();
	             	tsm.selectRange (lastSortedRange.width, lastSortedRange.height, table, stc, ParCoordUI.Combinator.FILTER);
				}
			}
		...
		public void mouseDragged (final MouseEvent mEvent) {
            ...
            if (dragCol == pressedCol && isDragging) {
            	...
            	final Dimension sortedIndicesRange = new Dimension ();
             	...
             	final ParCoordUI.Combinator comb = (mEvent.isAltDown() ? ParCoordUI.Combinator.FILTER : ParCoordUI.Combinator.ADD);
            	if (lastSortedRange.width == -1 || !lastSortedRange.equals (sortedIndicesRange)) {
            		final String intro = (comb == ParCoordUI.Combinator.FILTER ? FILTER : SELECT);
            		...
            		if (lastSortedRange.width != sortedIndicesRange.width) {
            			...
            			tsm.selectRange (start, end - 1, table, stc, select ? comb : ParCoordUI.Combinator.FILTER);
            		}
            			...
            			tsm.selectRange (start + 1, end, table, stc, select ? comb : ParCoordUI.Combinator.FILTER);
            		}
	...
	void setTooltip (final String intro, final Object val, final Object valUpper, 
    	...
    	final String label2 = val.equals (valUpper) ? "" : " - "+toString2 (valUpper, stc, viewColIndex);
...
//  Installation
    ...
    protected void installKeyboardActions() {
        LazyActionMap.installLazyActionMap ((JParCoord)table, ParCoord_Old_UI.class, "JParCoord.actionMap");
...
//  Uninstallation
    ...
    protected void drawBrushing (final ListSelectionModel brushModel, final boolean wasEmpty) {
    	if (!brushModel.isSelectionEmpty() || !wasEmpty) {
		    if ((lineDrawCounter.width == BACKGROUND && lineDrawCounter.height == 0)
		    		|| !(lineDrawCounter.width <= UBER_SELECTED)) {
		    	lineDrawCounter.setSize (END - 1, 0);
		    }
		    table.repaint ();
		}
	...
	public void paint (final Graphics graphics, final JComponent comp) {
    	...
    	//((JParCoord)jTable).allowPartialRepainting (!nonScreenOutput);
    	if (nonScreenOutput) {
    		((JParCoord)jTable).setRedrawAll (true);
    	...
    	final boolean redrawAll = ((JParCoord)table).isRedrawAll();	
    	final boolean redrawSelections =  ((JParCoord)table).isRedrawSelections();	
    	
    	LOGGER.info ("ra: "+redrawAll+", rs: "+redrawSelections);
    	...
    	if (redrawAll) {
    		resetPartialRepaintSettings ();
    		LOGGER.debug ("rd: "+redrawAll);
	    	...
	    	drawingStates.set (SELECTED, true);
	    	drawingStates.set (UBER_SELECTED, true);
	    	((JParCoord)table).setRedrawAll (false);
    	}
    	
       	LOGGER.info ("ldc: "+lineDrawCounter+", ds: "+drawingStates);
    ...
    private void resetPartialRepaintSettings () {
    	lineDrawCounter.setSize (END - 1, 0);
    	clearDrawParameters ();
    }
    ...
    public void drawData (final Graphics graphics, final int cMin, final int cMax) {
    	...
    	if (table.getColumnModel().getColumnCount() > 0) {
    		final boolean completedPaintingLines = paintPolylines (graphics, cMin, cMax, drawingStates, drawingAreas);
	    	paintLayers (graphics);  
	    	
	        if (completedPaintingLines) {
	        	if (drawingStates.get (0)) {
	        		nonBrushedLastDrawnAntiAliased = antiAlias;
	        	}
	        	clearDrawParameters ();
	        }
	        else {
	        	...
	        	SwingUtilities.invokeLater (
		        	new Runnable () {
						@Override
						public void run() {
							table.repaint (clip.x, clip.y, clip.width, clip.height);
						}		
					}
	        	);
	        }
	...
	public boolean paintPolylines (final Graphics graphics, final int cMin, final int cMax,
			final BitSet drawStates, final Graphics[] drawingAreas) {

		LOGGER.debug ("ldc: "+lineDrawCounter);
		...
		final boolean emptyUberSelect = !this.uberSelectionExists();

		final long nano = System.nanoTime();
		boolean timeUp = false; // set to true when 'x'ms of drawing has elapsed
		final boolean partialRepaintAllowed = !nonScreenOutput; //((JParCoord)table).isPartialRepaintingAllowed();
		final boolean fullRefresh = visibleRectNoInsets.equals (graphics.getClipBounds());
		...
		//System.err.println ("Uber: "+((JParCoord)table).getUberSelection());
		...
		for (cycle = lineDrawCounter.width; cycle >= 0 && !timeUp; cycle--) {
				
			for (int pCounter = 0; pCounter < primeImagePrompts.length; pCounter++) {
				final int promptStage = primeImagePrompts [pCounter];
				if (cycle == promptStage && lineDrawCounter.height == 0 && displayLayers.get (promptStage)) {
			...
			final int cycle2 = svgOutput ? (END - 1) - cycle : cycle;
			...
			if (displayLayers.get (cycle2) && gDrawable != null) {
				...
				for (viewRow = lineDrawCounter.height; viewRow < size && !timeUp; viewRow++) {
					...
					if (viewRow % 100 == 0) {
						timeUp = partialRepaintAllowed && (System.nanoTime() - nano) > paintTimeOut;
					}
					
					if (rowPresentLayers.get (cycle2)) {
						//int modeln = table.convertRowIndexToModel (viewRow);
						//if (uberSelectionModel != null) {
						//	uberSelected = uberSelectionModel.isSelectedIndex (modeln);
						//	gDrawable.setColor (uberSelected ? uberSelectionColour : drawColour);
						//}
				...
				lineDrawCounter.height = timeUp ? viewRow : 0;
				LOGGER.debug ("viewRow: "+viewRow+", size: "+size+", ldc: "+lineDrawCounter);
			}
			else {
				lineDrawCounter.height = 0;
			}	
		}
		
		// bump up cycle if the loop stopped because timeup occurred;
		if (timeUp) {
			cycle++;
		}
		final boolean finished = (cycle == -1); // loop finished as all lines drawn
		lineDrawCounter.setSize (finished ? BACKGROUND : cycle, finished ? 0 : lineDrawCounter.height);	// reset or store for next time
		

		LOGGER.debug ("ldc leaving state: "+lineDrawCounter);
        LOGGER.info ("paint lines: "+((System.nanoTime() - nanoTime)/1E6)+" ms.");

        translateDrawingAreas (drawingAreas, visibleRectNoInsets, true);
        
        return (lineDrawCounter.height == 0 && lineDrawCounter.width == BACKGROUND);
	}
    ...
    private void paintLabels (final Graphics graphics, final int column, final int x) {
    	...
    	final double jump = (disSize == 0 ? table.getRowHeight() : Math.max (table.getRowHeight(), (double)visibleRect.height / (double)Math.max (1, disSize - 1)));
    	final int xx = x;
